                    ...
                    if(mdp1S.equals(mdp2S)){
                        if(mdp1S.length() > 5){
                            if (!mdp1S.equals(user.getMotDePasse())) {
                                ...
                                DocumentReference userRef = db.collection(ConnectionActivity.COLLECTION_NAME).document(userS);
                                userRef.get().addOnSuccessListener(new OnSuccessListener<DocumentSnapshot>() {
                                    ...
                                    public void onSuccess(DocumentSnapshot documentSnapshot) {
                                        if(documentSnapshot.exists() && !userS.equals(user.getLogin())){
                                            ...
                                            db.collection(ConnectionActivity.COLLECTION_NAME).document(userS).set(userColl)
